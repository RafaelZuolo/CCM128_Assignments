...
import edu.princeton.cs.algs4.*;
 ...
 public class KMP {
	...
	public Iterable<Integer> findAll(String txt) {
		// simulate operation of DFA on text
		Queue<Integer> hits = new Queue<Integer>();
        int M = pat.length();
        int N = txt.length();
        int i, j;
        for (i = 0, j = 0; i < N; i++) {
            j = dfa[txt.charAt(i)][j];
			if (j == M) {
				hits.enqueue(i - M + 1);   // hit found
				j = 0;                 // reset DFA
				// retornamos no texto para ler o hit uma letra deslocada
				// e assim colocar o DFA no estado apropriado
				i = i - M + 1;        
			}
		}
        if (hits.isEmpty()) hits.enqueue(N);
		return hits;
	}
    ...
    public static void main(String[] args) {
		...
		Iterable<Integer> iter = kmp1.findAll(txt);
