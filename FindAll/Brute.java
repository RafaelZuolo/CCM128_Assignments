...
public class Brute {
    ...
    ***************************************************************************/
	...
	public static Iterable<Integer> findAll(String pat, String txt) {
		int M = pat.length();
        int N = txt.length();
		Queue<Integer> hit = new Queue<Integer>();
        ...
        for (int i = 0; i <= N - M; i++) {
            ...
            if (j == M) hit.enqueue(i);            // found at offset i
        }
		if (hit.isEmpty()) hit.enqueue(N);     // Se nao achou nenhum padrao
        return hit;                            // depois de ter descoberto todos
    ...
    public static void main(String[] args) {
		...
		Iterable<Integer> occurrence = findAll(pat, txt);
