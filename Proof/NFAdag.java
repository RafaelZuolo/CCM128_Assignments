...
public class NFAdag { 
    ...
    public boolean recognizes(String txt) {
        I = txt.length();
        proof = new Digraph(G.V() * (I + 1));
        DirectedDFS dfs = new DirectedDFS(G, 0);
        Bag<Integer> pc = new Bag<Integer>();
        for (int v = 0; v < G.V(); v++)
            if (dfs.marked(v)) {
            pc.add(v);
            if (dfs.hasFather(v)) proof.addEdge(dfs.myFatherIs(v), v);
        }
        ...
        for (int i = 0; i < txt.length(); i++) {
            ...
            for (int v = 0; v < G.V(); v++) {
				if (dfs.marked(v)) 
					pc.add(v);
                if (dfs.hasFather(v)) 
                    proof.addEdge(dfs.myFatherIs(v) + G.V() * (i + 1), 
												 v  + G.V() * (i + 1));
                }
        ...
        BreadthFirstDirectedPaths bfdp = new BreadthFirstDirectedPaths(proof, 0);
        // check for accept state
        for (int v : pc)
            if (v == M) {
				for (int i : bfdp.pathTo(G.V() * (I + 1) - 1))
					StdOut.println(i % G.V());
				return true;
			}
        return false;
    } 
