/******************************************************************************
 *  para fazer o metodo printLevel(Node x), para evitar redundancias, precisamos saber
 * a altura da subarvore do Node x, por isso o metodo howDeep(Node x)
 * 
 * 
...
public class BST<Key extends Comparable<Key>, Value> {
 ...
 //*********************************************************************************************************AKI COMECA O MEU EP*/
    public int howDeep(){        // acessar externamente para fins de teste
        return howDeep(root);
    }
    
    private int howDeep(Node x){     // precisamos saber a altura da arvore
        return howDeep(x, 0);
    }
    
    private int howDeep(Node x, int i){    
        if (x == null) return i;       // qnd atinjimos um null podemos ter atingido o fim da subarvore, entao retornamos i
        int l = howDeep(x.left, i+1);
        int r = howDeep(x.right, i+1);
        if (l < r) return r;               // queremos a altura da subarvore, entao devemos retornar a altura maior das
        else       return l;               // subarvores esquerda e direita
    }
    
    public void printLevel(){             // podemos acessar externamente para fins de teste
        printLevel(root);
    }
    
    private void printLevel(Node x){          // vamos criar um metodo que printa TODO um level especifico, 
        int N = howDeep(x);                   // dps chama-lo uma vez para cada level, de cima para baixo
        for(int i = 1; i <= N; i++)
            printLevel(x, i);
    }
    
    private void printLevel(Node x, int i){   // esse metodo navega ateh o level i e dps printa ele, e acessa todos os nodes do level de forma recu
        if (x == null) return;                // e acessa todos os nodes do level de forma recursiva, da esquerda para a direita.
        if (i == 0)
            StdOut.print(x.key + " ");
        else {
            i--;
            printLevel(x.left, i);
            printLevel(x.right, i);
        }
    }
   /****************************************************************************************************AKI TERMINA O MEU EP*/ 
    /**
