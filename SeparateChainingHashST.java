...
public class SeparateChainingHashST<Key, Value> {
    ...
    private int maxProbes;                        // maximo numero medio de probes
     ...
     */
    public SeparateChainingHashST(int M, int maxProbes) {
        this.M = M;
    ...
    private void resize(int chains) {
        SeparateChainingHashST<Key, Value> temp = new SeparateChainingHashST<Key, Value>(chains, maxProbes);
        for (int i = 0; i < M; i++) {
            for (Key key : st[i].keys()) {
                temp.put(key, st[i].get(key));
            }
        }
        this.maxProbes = temp.maxProbes;
        this.M  = temp.M;
        this.N  = temp.N;
        this.st = temp.st;
    }
   ...
   //     return (key.hashCode() & 0x7fffffff) % M;
    ...
    public void put(Key key, Value val) {
        ...
        // double table size if average length of list >= maxProbes
        if (N >= maxProbes) resize(2*M);    
    ...
    public void delete(Key key) {
        ...
        if (M > INIT_CAPACITY && N <= 2*M) resize(M/2);      //mantemos esse codigo
    } 
